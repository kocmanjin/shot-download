...
//import fjfi.kocman.ca.uol.aig.fftpack.RealDoubleFFT;
...
public class SigMod {    
    ...
    public static double[] plus(double[] data, double cislo) {
    ...
    /**
     * Zintegruje předaná data. Odstraní offset a při integrování vynásobí časovou složkou.
     *
     * @param time časový úsek mezidvěma daty
     * @param data integrovaná data
     * @param averLine počet vzorků, z kterých se má získat offset
     * @return zintegrovaná data
     */
...
//    }
    ...
    public static double[] averageFilter(double[] data, int size) {
        ...
        for (int i = size; i < data.length-size; i++) {
            double[] a = getSubArray(data, i, size);
            ret[i] = getAverage(a);
        }
        return ret;
    }
