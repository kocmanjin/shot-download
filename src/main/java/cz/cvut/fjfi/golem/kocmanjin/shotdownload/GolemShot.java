...
//    private static final String NIturbo_RTPath = "/DAS/0513NIturbo_RT.ON/input_data.txt";
    ...
    private int UbLimit = -1;
    private int UcdLimit = -1;
    private double pressure = -1;
    private double plasmaStart = -1;
    private double plasmaEnd = -1;
    private double breakdownVoltage = -1;
    private double breakdownMagField = -1;
    private double Tcd = -1;
    ...
    public GolemShot(int shotNo) {
        this.shotNo = shotNo;
        homeDir = IOFile.getFile(directory + File.separator + shotNo + "/");
    }
    ...
    public double[][] getPlasmaCurrent() {
        if (plasmaCurrent == null) {
            plasmaCurrent = getData(plasmaCurrentPath);
        }
        if (saveAnalyses) {
            IOFile.saveData(homeDir.getPath() + "/Iplasma.txt", plasmaCurrent);
        }
    ...
    public double[][] getPapouchST() {
    	if (papouchST== null) {
    		papouchST = getData(papouchSTPath);
    	}
//    	if (saveAnalyses) {
//    		IOFile.saveData(homeDir.getPath() + "/Ichamber.txt", chamberCurrent);
//    	}
    ...
    public double[][] getPapouchJI() {
    	if (papouchJI== null) {
    		papouchJI = getData(papouchJIPath);
    	}
//    	if (saveAnalyses) {
//    		IOFile.saveData(homeDir.getPath() + "/Ichamber.txt", chamberCurrent);
//    	}
    ...
    public double[][] getTorMagFieldDer() {
        if (torMagFieldDer == null) {
            torMagFieldDer = getData(TorMagFieldDerPath);
        }
...
//                    mirnovCoilsIntegrClean[4][i] += quadr[1][i]*5.62e-6;
    ...
    public double[][] getUloop() {
        if (Uloop == null) {
            Uloop = getData(UloopPath);
        }
        if (saveAnalyses) {
            IOFile.saveData(homeDir.getPath() + "/Uloop.txt", Uloop);
        }
        return Uloop;
    }
...
//        return max;
    ...
    private double[][] getData(String name) {
        ...
        File file = new File(homeDir.getPath() + "//" + name);
        if (!file.exists()) {
            data = IOFile.loadDataFromURL(prefix + shotNo + name);
